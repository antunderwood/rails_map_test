try{ jQuery(document).ready(function() {
    ...
    if (GBrowserIsCompatible()) {
          ...
          if(location){
            ...
            function marker_click(marker){
              return GEvent.addListener(marker, "click", function() {
                jQuery.get('/rails_map_test/map/show_location_info?location%5Blatitude%5D=' + marker.getLatLng().lat() + '&location%5Blongitude%5D=' + marker.getLatLng().lng(), function(data) {
      ...
      alert('Your browser be old, it cannot run google maps!');
    }
  });
}
catch (e) {
